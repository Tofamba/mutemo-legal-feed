...
async def _push_news_item(item: NewsItem, client: httpx.AsyncClient) -> bool:
    ...
    form_data = {
        ...
        "summary": item.markdown_summary[:500] if item.markdown_summary else "",
        "source_url": item.url,
        "scraped_at": item.scraped_at.isoformat() if getattr(item, "scraped_at", None) else "",
    }

    try:
        resp = await client.post(url, data=form_data, headers=_build_headers())
        if resp.status_code in (200, 201):
            logger.info(f"[pusher] ✓ News item pushed: {item.url}")
            state.increment_pushed()
            return True
        else:
        ...
        logger.error(f"[pusher] News push error: {e}")
        return False
